...
<#
.SYNOPSIS
    Windows installer for NexusCore via Docker Desktop.
#>
param(
    [switch]$SkipStart
)
...
Write-Host "=== Installing NexusCore ===" -ForegroundColor Cyan
...
if (-not $SkipStart) {
    Write-Host "Building and starting NexusCore on port 5173..." -ForegroundColor Green
    Invoke-Expression "$composeCmd up --build -d"
    Write-Host "NexusCore installed. Access: http://localhost:5173" -ForegroundColor Green
} else {
